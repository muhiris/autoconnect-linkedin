Linkedin = {
  ...
  sendInvites: function (data, config) {
    ...
    if (config.maxRequests == 0) {
      ...
      if (config.addNote && config.note) {
        ...
        console.debug(
          "DEBUG: clicking 'Send without note' in " + config.actionDelay + " ms"
        );
        setTimeout(
  ...
  clickSendWithoutNote: function (data, config) {
    // Find the "Send without a note" button by its span text
    var spans = document.querySelectorAll("span.artdeco-button__text");
    var sendWithoutNoteButton = Array.from(spans).find(
      (span) => span.textContent.trim() === "Send without a note"
    );

    if (sendWithoutNoteButton) {
      console.debug("DEBUG: clicking 'Send without a note' button");
      sendWithoutNoteButton.parentElement.click();
      setTimeout(() => this.handlePostSend(data, config), config.actionDelay);
    } else {
      console.warn("WARN: 'Send without a note' button not found!");
      setTimeout(() => this.handlePostSend(data, config), config.actionDelay);
    }
  },
  clickAddNote: function (data, config) {
    var spans = document.querySelectorAll("span.artdeco-button__text");
    var addNoteButton = Array.from(spans).find(
      (span) => span.textContent.trim() === "Add a note"
    );

    if (addNoteButton) {
      console.debug("DEBUG: clicking add a note button to paste note");
      addNoteButton.parentElement.click();
  ...
  pasteNote: function (data, config) {
    ...
    setTimeout(() => this.clickSendInvite(data, config), config.actionDelay);
  },
  clickSendInvite: function (data, config) {
    ...
    if (sendButton && sendButton[0]) {
      console.debug("DEBUG: clicking send button to send invite");
      sendButton[0].click();
      setTimeout(() => this.handlePostSend(data, config), config.actionDelay);
    } else {
      console.warn("WARN: Send button not found!");
      setTimeout(() => this.handlePostSend(data, config), config.actionDelay);
    }
  },
